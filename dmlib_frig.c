 ...
 */
    ...
    if ((cur_frame->reverse_sense ^ dir_flag) != lp->dir_flag)
    {
        fprintf(frp->ofp,
...
struct dm_context * dp;
    ...
    unsigned short int from, to;
        ...
        if (from == 59097 || to == 59097)
        {
            ...
            dp = ((struct dm_context *) (frp->app_ptr));
            strcpy(dp->user_name, "shatocv");  /* UNIX User     */
            strcpy(dp->password, "ipswich");  /* UNIX User     */
            strcpy(dp->function_name, "Unknown");            /* Description   */
            strcpy(dp->program_name,"clxso001n");
            for (i = 0; i < 2; i++)
            {
                frp->tran_cnt[i] = 0;
                frp->tran_len[i] = 0;
