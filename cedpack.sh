#!/bin/bash
# cedpack.sh - pick out the useful data from the trace.
#
set -x
gawk 'BEGIN { cnt = 0}
{
...
#
 print $1 "|" $7 "|" $2 "|" $3 "|" $5
}' tracetim.dat | gawk -F "|" 'BEGIN { see_cnt = 0 
OFMT = "%13.6f"
CONVFMT = "%13.6f"
run_time = 0}
#
...
# Re-output the message with a host and network time contribution
            print $1 "|" run_time "|" lab[s] "|1|0|" $5 "|0|" ht "|0|" nt "|0"
#
...
# If the message is an acknowledgement only, clear the outstanding flag
            ...
            lastt[s] = run_time
            print $1 "|" run_time "|" lab[s] "|0|1|0|" $5 "|0|" ht "|0|" nt
    ...
    {
        s = ini_sess()
        print $1 "|" run_time "|" lab[s] "|1|0|" $5 "|0|0|0|0"
    }
}
END {
