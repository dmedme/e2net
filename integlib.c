...
{
register struct integ_sess * rop = (struct integ_sess *) frp->app_ptr;

    if (rop != (struct integ_sess *) NULL)
    {
...
static struct bm_table * bp;
    ...
    if (bp == (struct bm_table *) NULL)
        bp = bm_compile("[1;1H[1m ");
    if ((!dir_flag) ^ f->reverse_sense) /* User Input */
    {
        if (((struct integ_sess *)(f->app_ptr))->last_input !=
                  (unsigned char *) NULL)
        {
/*
 ...
 */
            ...
            if (*p1 == '\0')
                *p1 = '}';
            p1++;
            for (i = 10, p2 = ((struct integ_sess *)(f->app_ptr))->last_input;
                   i > 0 && *p2 != '\0';
                            p1++, p2++, i--)
                 *p1 = *p2;
            *p1 = '}';
            p1++;
            if (((struct integ_sess *)(f->app_ptr))->last_output !=
                  (unsigned char *) NULL)
            {
                for (i = 10, p2 = ((struct integ_sess *)(f->app_ptr))->last_output;
                       i > 0 && *p2 != '\0';
                            p1++, p2++, i--)
                     *p1 = *p2;
            }
            *p1 = '\0';
            ...
            free(((struct integ_sess *)(f->app_ptr))->last_input);
            ((struct integ_sess *)(f->app_ptr))->last_input =
                     (unsigned char *) NULL;
        }
 ...
 */ 
        ...
        if ( ((struct integ_sess *)(f->app_ptr))->last_output != (char *) NULL)
             free(((struct integ_sess *)(f->app_ptr))->last_output);
        if (f->top[dir_flag] != f->hold_buf[dir_flag])
            ((struct integ_sess *)(f->app_ptr))->last_output =
                     pipe_stuff(f->hold_buf[dir_flag],
                          (f->top[dir_flag] - f->hold_buf[dir_flag]));
    }
    return;
}
int integ_app_recognise(frp)
struct frame_con * frp;
{
static int sess_seq;
char fname[28];
    ...
    unsigned short int from, to;
        ...
        if (from == 23 || to == 23)
        {
            ...
            if (from == 23)
                frp->reverse_sense = 1;
            frp->gap = 0;
            sprintf(fname,"integ_%d",sess_seq++);
            if ((frp->ofp = fopen(fname,"wb")) == (FILE *) NULL)
                 frp->ofp = stdout;
            frp->app_ptr = (char *) calloc(sizeof(struct integ_sess),1);
            return 1;
        }
    }
    return 0;
