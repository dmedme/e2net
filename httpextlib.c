/*
 * Scan a snoop file and pull out the Web Server and ORACLE Forms elements
 */
...
enum http_state { E2_HEADER, E2_BODY, E2_CHUNK };
/*
...
struct ora_web_sess {
    ...
    unsigned char * gday;
    int pragma_cnt[2];
    enum http_state http_state[2];
    int plain_flag;
    int to_go[2];
    unsigned char * f_enc_dec[2];        /* Encrypt/Decrypt control blocks */
};
static unsigned char * tunnel_f_enc_dec[2];
static int tunnel_pragma_cnt[2];
...
int i;
    ...
    if ((x = getenv("E2_ORA_TUNNEL_PORTS")) != (char *) NULL)
    {
        for (x = strtok(x," "); x != (char *) NULL; x = strtok(NULL, " "))
        {
            if ((i = atoi(x)) > 0 && i < 65536)   
            {
                web_match_add(match_port, &match_cnt, i);
                web_match_add(ora_tunnel_port, &ora_tunnel_cnt, i);
            }
        }
    }
 ...
 */
    ...
    if (frp->prot == E2_TCP)
    {
        ...
        if (i)
        {
            ...
            ((struct ora_web_sess *)(frp->app_ptr))->pragma_cnt[0] = 0;
            ((struct ora_web_sess *)(frp->app_ptr))->pragma_cnt[1] = 0;
            ((struct ora_web_sess *)(frp->app_ptr))->to_go[0] = 0;
            ((struct ora_web_sess *)(frp->app_ptr))->to_go[1] = 0;
            ((struct ora_web_sess *)(frp->app_ptr))->http_state[0] = E2_HEADER;
            ((struct ora_web_sess *)(frp->app_ptr))->http_state[1] = E2_HEADER;
            ((struct ora_web_sess *)(frp->app_ptr))->kept_msg[0] =
                        (unsigned char *) NULL;
            ((struct ora_web_sess *)(frp->app_ptr))->kept_msg[1] =
                            (unsigned char *) NULL;
            ((struct ora_web_sess *)(frp->app_ptr))->f_enc_dec[0] =
                        (unsigned char *) NULL;
            ((struct ora_web_sess *)(frp->app_ptr))->f_enc_dec[1] =
                            (unsigned char *) NULL;
            ((struct ora_web_sess *)(frp->app_ptr))->gday =
                            (unsigned char *) NULL;
            if (web_match_true(ora_web_port, ora_web_cnt, from, to))
                ((struct ora_web_sess *)(frp->app_ptr))->tunnel_flag = 0;
            else
            if (web_match_true(ora_tunnel_port, ora_tunnel_cnt, from, to))
            {
                ((struct ora_web_sess *)(frp->app_ptr))->tunnel_flag = 1;
                ((struct ora_web_sess *)(frp->app_ptr))->f_enc_dec[0] =
                        tunnel_f_enc_dec[0];
                ((struct ora_web_sess *)(frp->app_ptr))->f_enc_dec[1] =
                        tunnel_f_enc_dec[1];
                ((struct ora_web_sess *)(frp->app_ptr))->pragma_cnt[0] =
                        tunnel_pragma_cnt[0];
                ((struct ora_web_sess *)(frp->app_ptr))->pragma_cnt[1] =
                        tunnel_pragma_cnt[1];
                ((struct ora_web_sess *)(frp->app_ptr))->gday =
                            (unsigned char *) NULL;
            }
...
struct ora_web_sess * ap = (struct ora_web_sess *) (frp->app_ptr);
    ...
    if ((!dir_flag) ^ frp->reverse_sense)
    {
           ...
           && !memcmp(frp->hold_buf[dir_flag], "GDay", 4))
        {
            ...
            memcpy( ap->gday, frp->hold_buf[dir_flag] + 4, 4);
            fputs("\\D:B:", frp->ofp);
            ip_dir_print(frp->ofp, frp, dir_flag);
            fputs("\\\n", frp->ofp);
        ...
        {
            if (ap->f_enc_dec[dir_flag] != (unsigned char *) NULL)
                block_enc_dec(ap->f_enc_dec[dir_flag],
                       frp->hold_buf[dir_flag],
                       frp->hold_buf[dir_flag],
                    frp->top[dir_flag] - frp->hold_buf[dir_flag]);
            if (verbose)
    ...
    else
    {
        if (ap->tunnel_flag == 0
           ...
           && (frp->top[dir_flag] - frp->hold_buf[dir_flag]) > 4
           && !memcmp(frp->hold_buf[dir_flag], "Mate", 4))
        {
            ap->f_enc_dec[dir_flag] =
                            ready_code( ap->gday, frp->hold_buf[dir_flag] + 4);
            ap->f_enc_dec[!dir_flag] =
                            ready_code( ap->gday, frp->hold_buf[dir_flag] + 4);
            if (both_ways)
            {
 ...
 */
static void do_http(frp, dir_flag)
struct frame_con * frp;
int dir_flag;
{
    cur_frame = frp;
    if ((!dir_flag) ^ frp->reverse_sense)
        http_client(frp, dir_flag);
    else
        http_server(frp, dir_flag);
    return;
}
/*
 * Process response from server to client
 */
static void http_server(frp, dir_flag)
struct frame_con * frp;
int dir_flag;
{
unsigned char * x;
...
struct ora_web_sess * ap = (struct ora_web_sess *) (frp->app_ptr);
    ...
    x1 = x;
    if (ap->http_state[dir_flag] == E2_HEADER)
    {
        ap->plain_flag = 1;
        for (;;)
        {
 ...
 */
            for (x2 = x1; x2 < top; x2++)
                if ( *x2 == '\n' && (x2 > x1 && *(x2 - 1) == '\r' ))
                    break;
            if (x2 >= top)
                goto reserve;               /* Need another piece */
            if (x2 == (x1 + 1))
            {
                if (ap->to_go[dir_flag] == -2)
                    ap->to_go[dir_flag] = 0;
                else
                {
                    x1 += 2;
                    if (ap->to_go[dir_flag] == -1)
                        ap->http_state[dir_flag] = E2_CHUNK;
                    else
                        ap->http_state[dir_flag] = E2_BODY;
                    if (both_ways)
                    {
                        fputs("\\A:B:", frp->ofp);
                        ip_dir_print(frp->ofp, frp, dir_flag);
                        fputs("\\\n", frp->ofp);
                        (void) fwrite(x, 1, (x1 - x), frp->ofp);
                    }
                    if (x1 < top)
                    {
                        ...
                        top =  x + ap->len[dir_flag];
                        x1 = x;
                        goto get_rest;       /* End of the HTTP header */
                    }
                    else
                    {
                        if ( ap->kept_msg[dir_flag] != (unsigned char *) NULL)
                        {
                            free( ap->kept_msg[dir_flag]);
                            ap->len[dir_flag] = 0;
                            ap->kept_msg[dir_flag] = (unsigned char *) NULL;
                        }
                        if (ap->to_go[dir_flag] == 0 && both_ways)
                            fputs("\\A:E\\\n", frp->ofp);
                        return;
                    }
                }
            ...
            if (!ap->to_go[dir_flag]
             && (x2 > (x1 + 12))
             && !strncmp(x1, "HTTP/1.1 100", 12))
            {
                ap->to_go[dir_flag] = -2;
            }
            else
            if ( (x2 >= (x1 + 17))
             && ( !strncasecmp(x1, "Connection: close", 17)))
            {
                ap->to_go[dir_flag] = -3;
            }
            else
            ...
            {
                i = atoi(x1 + 8);
                if (i <= ap->pragma_cnt[dir_flag]
                  && (ap->gday != NULL
                  || ap->f_enc_dec[0] != NULL))
                    return;
                ap->pragma_cnt[dir_flag] = i;
                tunnel_pragma_cnt[dir_flag] = i;
            }
            else
            ...
            if (!ap->to_go[dir_flag] && 
                   x2 > (x1 + 16) && !strncasecmp(x1, "Content-Length: ", 16))
                ap->to_go[dir_flag] = atoi(x1 + 16);
            else
            if (!ap->to_go[dir_flag] && 
                x2 >= (x1 + 26) && !strncasecmp(x1,
 ...
 * - Close indicated; just read
 * - Chunking selected; read until all done.
 */
        if (ap->to_go[dir_flag] == -1)
        {
/*
 * Chunked data; need chunk size
 */
            for (;;)
            {
            unsigned int n;
                ...
                if (x1 > top - 4)
                {
                    if (x != x1)
                        adjust_buf(frp, dir_flag, x1);
                    return;   /* No room for chunk size */
                }
                (void) sscanf(x1, "%x%n", &ap->to_go[dir_flag], &n);
                fprintf(stderr,
                       "Chunk details: x1=[%.*s] ap->to_go[dir_flag]=%d\n",
                             n + ((10 < ap->to_go[dir_flag]) ? 10
                             : 2 + n + ap->to_go[dir_flag]),
                              x1, ap->to_go[dir_flag]);
                if (ap->to_go[dir_flag] == 0)
                {
                    if (both_ways)
                        fputs("\\A:E\\\n", frp->ofp);
                    x1 += (n + 2);
                    while(x1 < top && (*x1 == '\r' || *x1 == '\n'))
                        x1++;
                    adjust_buf(frp, dir_flag, x1);
                    ap->http_state[dir_flag] = E2_HEADER;
                    ap->http_state[!dir_flag] = E2_HEADER;
                    return;
                }
                x2 = x1 + ap->to_go[dir_flag] + n + 4;
                x1 += 2 + n;
                if (x2 > top)
                {
                    if (x != x1)
                ...
                {
                    ap->f_enc_dec[dir_flag] =
                            ready_code( ap->gday, x1 + 4);
                    ap->f_enc_dec[!dir_flag] =
                            ready_code( ap->gday, x1 + 4);
                    tunnel_f_enc_dec[dir_flag] =
                           ap->f_enc_dec[dir_flag];
                    tunnel_f_enc_dec[!dir_flag] =
                           ap->f_enc_dec[!dir_flag];
                    if (x2 > x1 + 8)
                        block_enc_dec(ap->f_enc_dec[dir_flag],
                           x1 + 8, x1 + 8, ap->to_go[dir_flag] - 8);
                }
                else
                if (ap->f_enc_dec[dir_flag] != (unsigned char *) NULL
                   && (ap->plain_flag == 0))
                {
                    block_enc_dec(ap->f_enc_dec[dir_flag],
                       x1, x1, ap->to_go[dir_flag]);
                    if (verbose)
                    {
                        fputs("______________\n", frp->ofp);
                        for (x4 = x1; x4 < x2;)
                            x4 = forms60_handle(frp->ofp, x4, x2, 1);
                        fputs("==============\n", frp->ofp);
                    }
                }
                if (both_ways)
                    (void) gen_handle(frp->ofp, x1, x2, 1);
                x1 = x2;
            }
        }
        else
        if (ap->to_go[dir_flag] > 0)
        {
              ...
              && !strncmp((char *) x1,"Mate",4))
            {
                ...
                ap->f_enc_dec[!dir_flag] =
                            ready_code( ap->gday, x1 + 4);
                tunnel_f_enc_dec[dir_flag] = ap->f_enc_dec[dir_flag];
                tunnel_f_enc_dec[!dir_flag] = ap->f_enc_dec[!dir_flag];
                if (x2 > x1 + 8)
                    block_enc_dec(ap->f_enc_dec[dir_flag],
                       x1 + 8, x1 + 8, ap->to_go[dir_flag] - 8);
            ...
            if (both_ways)
                (void) gen_handle(frp->ofp, x1, x2, 1);
            if (ap->http_state[dir_flag] != E2_CHUNK)
            {
                if (both_ways)
                    fputs("\\A:E\\\n", frp->ofp);
                ap->http_state[dir_flag] = E2_HEADER;
 ...
 */
               ...
               && ap->to_go[dir_flag] > 0)
            {
                ...
                if (verbose)
                {
                    fputs("______________\n", frp->ofp);
                    for (x4 = x1; x4 < top;)
                        x4 = forms60_handle(frp->ofp, x4, top, 1);
                    fputs("==============\n", frp->ofp);
                }
            }
            if (both_ways)
...
reserve:
    if (top == frp->top[dir_flag])
    {
        ap->kept_msg[dir_flag] = (unsigned char *)
                      malloc( (top - x1));
        memcpy(ap->kept_msg[dir_flag], x1, (top - x1));
        ap->len[dir_flag] = (top - x1);
        frp->top[dir_flag] = frp->hold_buf[dir_flag];
    }
    return;
}
 ...
 */
static void http_client(frp, dir_flag)
struct frame_con * frp;
int dir_flag;
{
unsigned char * x;
