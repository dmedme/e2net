/* 
   ...
   DME      18th Jun 2001 - Changed to pass in the output file descriptor
   ===
   Derived from:

     print-smb.c	Mon Jun 28 22:06:55 1999
*/
...
static void trans2_findfirst(FILE * fp,
   uchar *param,uchar *data,int pcnt,int dcnt)
{
  char *fmt;
  ...
  fdata(fp, param,fmt,param+pcnt);
  if (dcnt) {
    fputs("data:\n", fp);
    print_data(fp, data,dcnt);
  }
}

static void trans2_qfsinfo(FILE *fp, uchar *param,uchar *data,int pcnt,int dcnt)
{
  static int level=0;
  char *fmt="";
...
static void print_trans2(FILE *fp,
uchar *words,uchar *dat,uchar *buf,uchar *maxbuf)
{
  ...
  fprintf(fp, "%s param_length=%d data_length=%d\n",
	 fn->name,pcnt,dcnt);

  if (request) {
    if (CVAL(words,0) == 8) {
      fdata(fp,words+1,"Trans2Secondary\nTotParam=[d]\nTotData=[d]\nParamCnt=[d]\nParamOff=[d]\nParamDisp=[d]\nDataCnt=[d]\nDataOff=[d]\nDataDisp=[d]\nHandle=[d]\n",maxbuf);
      return;	    
    } else {
      fdata(fp, words+1,"TotParam=[d]\nTotData=[d]\nMaxParam=[d]\nMaxData=[d]\nMaxSetup=[d]\nFlags=[w]\nTimeOut=[D]\nRes1=[w]\nParamCnt=[d]\nParamOff=[d]\nDataCnt=[d]\nDataOff=[d]\nSetupCnt=[d]\n",words+1+14*2);
      fdata(fp,data+1,"TransactionName=[S]\n%",maxbuf);
    }
  ...
  if (fn->descript.fn) {
    fn->descript.fn(fp, param,data,pcnt,dcnt);
  } else {
    fdata(fp, param,f1?f1:(uchar*)"Paramaters=\n",param+pcnt);
    fdata(fp, data,f2?f2:(uchar*)"Data=\n",data+dcnt);      
  }
}


static void print_browse(FILE *fp, uchar *param,int paramlen,uchar *data,int datalen)
{
  uchar *maxbuf = data + datalen;
  int command = CVAL(data,0);

  fdata(fp, param,"BROWSE PACKET\n|Param ",param+paramlen);

  switch (command) {
  case 0xF:
    data = fdata(fp, data,"BROWSE PACKET:\nType=[B] (LocalMasterAnnouncement)\nUpdateCount=[w]\nRes1=[B]\nAnnounceInterval=[d]\nName=[n2]\nMajorVersion=[B]\nMinorVersion=[B]\nServerType=[W]\nElectionVersion=[w]\nBrowserConstant=[w]\n",maxbuf);
    break;
  ...
  default:
    data = fdata(fp, data,"Unknown Browser Frame ",maxbuf);
    break;
  }
}


static void print_ipc(FILE *fp, uchar *param,int paramlen,uchar *data,int datalen)
{
  int command = SVAL(param,0);
  if (paramlen)
    fdata(fp, param,"Command=[w]\nStr1=[S]\nStr2=[S]\n",param+paramlen);
  if (datalen)
    fdata(fp, data,"IPC ",data+datalen);
}


static void print_trans(FILE *fp, uchar *words,uchar *data1,uchar *buf,uchar *maxbuf)
{
  uchar *f1,*f2,*f3,*f4;
  uchar *data,*param;
  ...
  fdata(fp, words+1,f1,MIN(words+1+2*CVAL(words,0),maxbuf));
  fdata(fp, data1+2,f2,maxbuf - (paramlen + datalen));

  if (!strcmp(data1+2,"\\MAILSLOT\\BROWSE")) {
    print_browse(fp, param,paramlen,data,datalen);
    return;
  }

  if (!strcmp(data1+2,"\\PIPE\\LANMAN")) {
    print_ipc(fp, param,paramlen,data,datalen);
    return;
  }

  if (paramlen) fdata(fp, param,f3,MIN(param+paramlen,maxbuf));
  if (datalen) fdata(fp, data,f4,MIN(data+datalen,maxbuf));
}



void print_negprot(FILE *fp, uchar *words,uchar *data,uchar *buf,uchar *maxbuf)
{
  uchar *f1=NULL,*f2=NULL;
  ...
  if (f1) 
    fdata(fp, words+1,f1,MIN(words + 1 + CVAL(words,0)*2,maxbuf));
  else
    print_data(fp, words+1,MIN(CVAL(words,0)*2,PTR_DIFF(maxbuf,words+1)));
  
  if (f2) 
    fdata(fp, data+2,f2,MIN(data + 2 + SVAL(data,0),maxbuf));
  else
    print_data(fp, data+2,MIN(SVAL(data,0),PTR_DIFF(maxbuf,data+2)));
    
}

void print_sesssetup(FILE *fp, uchar *words,uchar *data,uchar *buf,uchar *maxbuf)
{
...
print a SMB message
********************************************************************/
void print_smb(FILE *fp, uchar *buf,uchar *maxbuf)
{
  int command;
  uchar *words, *data;
  struct smbfns *fn;
...
"[P4]SMB Command   =  [B]\nError class   =  [BP1]\nError code    =  [d]\nFlags1        =  [B]\nFlags2        =  [B][P13]\nTree ID       =  [d]\nProc ID       =  [d]\nUID           =  [d]\nMID           =  [d]\nWord Count    =  [b]\n";
  ...
  fdata(fp, buf,fmt_smbheader,buf+33);

  if (CVAL(buf,5)) {
    int class = CVAL(buf,5);
    int num = SVAL(buf,7);
    fprintf(fp, "SMBError = %s\n",smb_errstr(class,num));
  }
  ...
  while (words && data)
	...
	f2 = fn->descript.rep_f2;
      ...
      if (fn->descript.fn) {
	fn->descript.fn(fp, words,data,buf,maxbuf);
      } else {
	if (f1) {
	  fputs("smbvwv[]=\n", fp);
	  fdata(fp, words+1,f1,words + 1 + wct*2);
	} else if (wct) {
	  int i;
	  int v;
	  fputs("smbvwv[]=\n", fp);
	  for (i=0;i<wct;i++) {
	    v = SVAL(words+1,2*i);
	    fprintf(fp, "smb_vwv[%d]=%d (0x%X)\n",i,v,v);
	  }
	}
	
	if (f2) {
	  fputs("smbbuf[]=\n", fp);
	  fdata(fp, data+2,f2,maxbuf);
	} else {
	  int bcc = SVAL(data,0);
	  fprintf(fp, "smb_bcc=%d\n",bcc);
	  if (bcc>0) {
	    fputs("smb_buf[]=\n", fp);
	    print_data(fp, data+2,MIN(bcc,PTR_DIFF(maxbuf,data+2)));
	  }
	}
      }
...
*/
void nbt_tcp_print(FILE *fp, uchar *data,uchar *maxbuf)
{
  ...
  case 0:    
    data = fdata(fp, data,"NBT Session Packet\nFlags=[rw]\nLength=[rd]\n",data+4);
    if (memcmp(data,"\377SMB",4)==0) {
      if (nbt_len>PTR_DIFF(maxbuf,data))
	fprintf(fp, "WARNING: Short packet. Try increasing the snap length (%d)\n",
	       PTR_DIFF(maxbuf,data));
      print_smb(fp, data,maxbuf>data+nbt_len?data+nbt_len:maxbuf);
    } else {
	    fputs("Session packet:(raw data?)\n", fp);
    }
    break;

  case 0x81:
    data = fdata(fp, data,"NBT Session Request\nFlags=[rW]\nDestination=[n1]\nSource=[n1]\n",maxbuf);
    break;

  case 0x82:
    data = fdata(fp, data,"NBT Session Granted\nFlags=[rW]\n",maxbuf);
    break;

  case 0x83:
    {
      int ecode = CVAL(data,4);
      data = fdata(fp, data,"NBT SessionReject\nFlags=[rW]\nReason=[B]\n",maxbuf);
      switch (ecode) {
      case 0x80: 
	fputs("Not listening on called name\n", fp); 
	break;
      case 0x81: 
	fputs("Not listening for calling name\n", fp); 
	break;
      case 0x82: 
	fputs("Called name not present\n", fp); 
	break;
      case 0x83: 
	fputs("Called name present, but insufficient resources\n", fp); 
	break;
      default:
	fprintf(fp, "Unspecified error 0x%X\n",ecode); 
	break;	  
      }
    }
    break;
  ...
  case 0x85:
    data = fdata(fp, data,"NBT Session Keepalive\nFlags=[rW]\n",maxbuf);
    break;

  default:
    fprintf(fp, "flags=0x%x\n", flags);
    data = fdata(fp, data,"NBT - Unknown packet type\nType=[rW]\n",maxbuf);
  }
  fputc('\n', fp);
  fflush(fp);
}
...
*/
void nbt_udp137_print(FILE *fp, uchar *data,uchar *maxbuf)
{
...

  {
    ...
    if (qdcount>100 || total>100) {
      fputs("Corrupt packet??\n", fp);
      return;
    }

    if (qdcount) {
      fputs("QuestionRecords:\n", fp);
      for (i=0;i<qdcount;i++)
	p = fdata(fp, p,"|Name=[n1]\nQuestionType=[rw]\nQuestionClass=[rw]\n#",maxbuf);
    }
	...
	p = fdata(fp, p,"Name=[n1]\n#",maxbuf);
	restype = RSVAL(p,0);
	p = fdata(fp, p,"ResType=[rw]\nResClass=[rw]\nTTL=[rD]\n",p+8);
	rdlen = RSVAL(p,0);
	fprintf(fp, "ResourceLength=%d\nResourceData=\n",rdlen);
	p += 2;
	if (rdlen == 6) {
	  p = fdata(fp, p,"AddrType=[rw]\nAddress=[b.b.b.b]\n",p+rdlen);
	} else {
	  if (restype == 0x21) {
	    int numnames = CVAL(p,0);
	    p = fdata(fp, p,"NumNames=[B]\n",p+1);
	    while (numnames--) {
	      char flags[128]="";
	      p = fdata(fp, p,"Name=[n2]\t#",maxbuf);
...
*/
void nbt_udp138_print(FILE *fp, uchar *data,uchar *maxbuf)
{
  startbuf = data;
  if (maxbuf <= data) return;
...
*/
void netbeui_print(FILE *fp, uchar *data,uchar *maxbuf)
{
  int len = SVAL(data,1);
  ...
  switch (command) {
  case 0xA: 
    data = fdata(fp, data,"NameQuery:[P1]\nSessionNumber=[B]\nNameType=[B][P2]\nResponseCorrelator=[w]\nDestination=[n2]\nSource=[n2]\n",data2);
    break;

  case 0x8:
    data = fdata(fp, data,"NetbiosDataGram:[P7]\nDestination=[n2]\nSource=[n2]\n",data2);
    break;

  case 0xE:
    data = fdata(fp, data,"NameRecognise:\n[P1]\nData2=[w]\nTransmitCorrelator=[w]\nResponseCorelator=[w]\nDestination=[n2]\nSource=[n2]\n",data2);
    break;

  case 0x19:
    data = fdata(fp, data,"SessionInitialise:\nData1=[B]\nData2=[w]\nTransmitCorrelator=[w]\nResponseCorelator=[w]\nRemoteSessionNumber=[B]\nLocalSessionNumber=[B]\n",data2);
    break;

  case 0x17:
    data = fdata(fp, data,"SessionConfirm:\nData1=[B]\nData2=[w]\nTransmitCorrelator=[w]\nResponseCorelator=[w]\nRemoteSessionNumber=[B]\nLocalSessionNumber=[B]\n",data2);
    break;

  case 0x16:
    data = fdata(fp, data,"NetbiosDataOnlyLast:\nFlags=[{|NO_ACK|PIGGYBACK_ACK_ALLOWED|PIGGYBACK_ACK_INCLUDED|}]\nResyncIndicator=[w][P2]\nResponseCorelator=[w]\nRemoteSessionNumber=[B]\nLocalSessionNumber=[B]\n",data2);
    break;

  case 0x14:
    data = fdata(fp, data,"NetbiosDataAck:\n[P3]TransmitCorrelator=[w][P2]\nRemoteSessionNumber=[B]\nLocalSessionNumber=[B]\n",data2);
    break;

  case 0x18:
    data = fdata(fp, data,"SessionEnd:\n[P1]Data2=[w][P4]\nRemoteSessionNumber=[B]\nLocalSessionNumber=[B]\n",data2);
    break;

  default:
    data = fdata(fp, data,"Unknown Netbios Command ",data2);
    break;
  }

  if (memcmp(data2,"\377SMB",4)==0) {
    print_smb(fp, data2,maxbuf);
  } else {
    int i;
    for (i=0;i<128;i++) {
      if (memcmp(&data2[i],"\377SMB",4)==0) {
	printf("found SMB packet at %d\n", i);
	print_smb(fp, &data2[i],maxbuf);
	break;
      }
    }
  }
...
*/
void ipx_netbios_print(FILE *fp, uchar *data,uchar *maxbuf)
{
