...
FILE *f, *fe, *fi;
    ...
    while ((ret = fread(&buf[0],sizeof(unsigned char),sizeof(snoop),f)) > 0)
    {
        ...
        fprintf(fe, "%d %d %u.%06d ",eth_pack_no, snoop.len, 
               snoop.secs_since_1970, snoop.musecs);
        if (snoop.len > 65536)
        {
            (void) fprintf(stderr,
        ...
        fprintf(fe, "%s ",ether_ntoa(&(eth.ether_shost)));
