/* ./smbutil.c    Wed Jun  9 00:06:33 1999 */
#include <sys/param.h>
#include <sys/time.h>
#include <sys/types.h>
...
time_t make_unix_date(void *date_ptr)
{
 ...
 t;
  ...
  interpret_dos_date(dos_date,&t.tm_year,&t.tm_mon,
    ...
    (void) date_val(&buf[0], "dd mm yyyy hh24:mi:ss", &x, &secs_since_1970);
    return (time_t) secs_since_1970;
#else
  t.tm_wday = 1;
  t.tm_yday = 1;
  t.tm_isdst = 0;
...
void print_asc(FILE *fp, unsigned char *buf,int len)
{
  int i;
  for (i=0;i<len;i++)
...
void print_data(FILE *fp, unsigned char *buf,int len)
{
  int i=0;
  if (len<=0) return;
  fprintf(fp, "[%03X] ",i);
  for (i=0;i<len;) {
    fprintf(fp, "%02X ",(int)buf[i]);
    i++;
    if (i%8 == 0) fputc(' ', fp);
    if (i%16 == 0) {      
      print_asc(fp, &buf[i-16],8); fputc(' ', fp);
      print_asc(fp, &buf[i-8],8); fputc('\n', fp);
      if (i<len) fprintf(fp, "[%03X] ",i);
    }
  }
  if (i%16) {
    int n;
    ...
    n = MIN(8,i%16);
    print_asc(fp, &buf[i-(i%16)],n); fputc(' ', fp);
    n = (i%16) - n;
    if (n>0) print_asc(fp, &buf[i-n],n); 
    fputc('\n', fp); 
  }
}


static void write_bits(FILE *fp, unsigned int val,char *fmt)
{
  char *p = fmt;
  int i=0;
...
uchar *fdata1(FILE * fp, uchar *buf,char *fmt,uchar *maxbuf)
{
  int reverse=0;
  ...
  while (*fmt && buf<maxbuf) {
    switch (*fmt) {
    case 'a':
      write_bits(fp, CVAL(buf,0),attrib_fmt);
      buf++; fmt++;
      break;

    case 'A':
      write_bits(fp, SVAL(buf,0),attrib_fmt);
      buf+=2; fmt++;
      break;
    ...
    bitfmt[l]=0;
    fmt = p+1;
    write_bits(fp, CVAL(buf,0),bitfmt);
    buf++;
    break;
      }
    ...
    if (x2) {
        fprintf(fp, "0x%08x:%08x",x2, x1);
    } else {
        fprintf(fp, "%d (0x%08x%08x)",x1, x2, x1);
    }
    buf += 8;
    fmt++;
    break;
    ...
    case 'S':
      {    
          fprintf(fp, "%.*s",PTR_DIFF(maxbuf,buf),unistr(buf, &len));
          buf += len;
          fmt++;
          break;
      }
    case 'Z':
      {    
    if (*buf != 4 && *buf != 2) 
      fprintf(fp, "Error! ASCIIZ buffer of type %d (safety=%d)\n",
         *buf,PTR_DIFF(maxbuf,buf));
    fprintf(fp, "%.*s",PTR_DIFF(maxbuf,buf+1),unistr(buf+1, &len));
    buf += len+1;
    fmt++;
    break;
      }
    ...
    case 1:
      ...
      buf += name_len(buf);
      fprintf(fp, "%-15.15s NameType=0x%02X (%s)",
    ...
    fprintf(fp, "%s",t?(long) asctime(localtime(&t)):(long) "NULL\n");
    fmt++; while (isdigit(*fmt)) fmt++;
    break;
      }
    default:
...
uchar *fdata(FILE *fp, uchar *buf,char *fmt,uchar *maxbuf)
{
  static int depth=0;
  char s[128];
  char *p;
